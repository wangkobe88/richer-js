...
// 构建钱包地址->代币数量映射（作为权重）
const walletVolumeMap = {};
walletDataComplete.forEach(w => {
  walletVolumeMap[w.address.toLowerCase()] = w.total_tokens || 0;
});

// 计算总权重
const totalVolume = Object.values(walletVolumeMap).reduce((a, b) => a + b, 0);
console.log('钱包数据已加载');
console.log(`  总钱包数: ${Object.keys(walletVolumeMap).length}`);
console.log(`  总交易代币数: ${totalVolume.toFixed(0)}`);
...
async function main() {
  ...
  console.log('基于人工标注质量的分析 v4（细致分类 + 交易量权重）');
  console.log('='.repeat(80));
  ...
  labeled.forEach(token => {
    ...
    participants.forEach(addr => {
      ...
      const weight = walletVolumeMap[addr.toLowerCase()] || 0; // 交易量作为权重
  ...
  // 计算相关性
  console.log('\n' + '='.repeat(80));
  console.log('钱包分类占比与质量的相关性分析（交易量加权）');
  console.log('='.repeat(80));
  ...
  const sortedByCorrelation = Object.entries(correlations)
    ...
    .sort((a, b) => Math.abs(b[1].point_biserial) - Math.abs(a[1].point_biseral));
  ...
  ['high_quality', 'mid_quality', 'low_quality'].forEach(quality => {
    ...
    detailedScheme.categories.forEach(cat => {
      ...
      if (avgRatio > 0.001) { // 只显示占比>0.1%的分类
  ...
  fs.writeFileSync(
    path.join(DATA_DIR, 'data/quality_volume_weighted_results.json'),
    JSON.stringify(result, null, 2)
  );

  console.log(`\n✅ 分析完成! 结果已保存到 data/quality_volume_weighted_results.json`);
}
