/**
 * 分析实验 afed3289-2f89-4da5-88f1-1468d61f8b3d 的人工判断数据与购买前检查特征的关系
 */
...
async function analyzeHumanJudgment() {
  ...
  const { data: tokens, error: tokensError } = await supabase
    ...
    .select('token_address, human_judges, token_symbol, discovered_at')
  ...
  if (tokens.length === 0) {
    console.log('没有找到人工判断数据，尝试查看所有代币...');
    const { data: allTokens, error: allTokensError } = await supabase
      .from('experiment_tokens')
      .select('token_address, token_symbol, human_judges')
      .eq('experiment_id', sourceExperimentId || experimentId)
      .limit(10);
    console.log(`总共有 ${allTokens?.length || 0} 个代币`);
    if (allTokens && allTokens.length > 0) {
      console.log('示例代币:', allTokens[0]);
    }
    return;
  }
