/**
 * Prompt构建器 - V7.22 Section-based架构
 * 根据实际获取的数据动态组装Prompt
 *
 * V7.22变更：
...
**1. 硬蹭/弱关联**：代币名称借用了推文中的词汇，但推文主体与代币主体不相关
   ...
   - ✅ **不是硬蹭（强关联）**：代币名直接引用推文中的核心产品/服务/概念名称
     - 示例：推文"硅基茶水间AI功能上线"，代币名"硅基茶水间" → 强关联（直接引用AI功能名称）
     - 示例：推文"AgentPay SDK发布"，代币名"AgentPay" → 强关联（直接引用产品名称）
     - 示例：推文"币安VIP升级"，代币名"币安VIP" → 强关联（直接引用服务名称）
   - ❌ **是硬蹭（弱关联）**：代币名只是借用推文中的词汇，但说的不是同一件事
     - 示例：推文"绿水青山"，代币名"绿水青山就是金山银山" → 硬蹭（推文说风景，代币说标语，主体不同）
     - 示例：推文"AI创作比赛获奖"，提到"AI meme"，代币名"AImeme" → 硬蹭（推文说比赛，代币说AI meme概念，主体不同）
     - 示例：推文"特斯拉发布新功能"，代币名"特斯拉" → 硬蹭（推文说特斯拉公司，代币说特斯拉这个名字，主体不同）
