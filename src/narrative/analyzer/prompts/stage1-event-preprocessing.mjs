...
- 普通KOL或一般知名机构（如小交易所发节日祝福 → 仍可判定为空洞内容）
- 知名但非"超大"的人物（如百万粉丝KOL → 按正常标准判断）

**⚠️ 有影响力用户对大IP的情感表达/认同/致敬不算空洞**：

如果推文作者/发言者是有一定影响力的用户（粉丝>1万、有认证、或已知身份如"币安天使志愿者"等），且内容是对超大IP/大品牌（CZ、何一、Binance、Elon等）的：
- 情感表达/认同/感谢（如"Binance been my first home"）
- 致敬/深度感悟（如引用CZ回忆录表达个人感悟）
- 正面评价/称赞（如表达对币安文化的认同）

这类内容**不算空洞**，应通过空洞内容检查。理由：
1. 有影响力的用户对大IP的情感表达，有引发大IP回复/转发的潜力
2. 这类内容在meme币生态中是常见的叙事来源（"拍马屁"博回复）
3. 内容有明确的主体（发言者）、明确的对象（大IP）、具体的情感/观点 → 满足"能定义出事件"的最低要求

示例：
- ✅ "Binance been my first home in this industry" → 对币安的深情认同，不算空洞
- ✅ 引用CZ回忆录表达个人感悟 → 有具体内容和观点，不算空洞
- ❌ "Binance 🚀🚀🚀" → 纯名称+表情，仍然空洞
...
⚠️ **必须提取eventSubject（事件主体）的影响力数据**：
- eventSubject本人的粉丝数、认证状态、头衔等
- 这是Stage 2判断分量等级（S/A/B/C级）的关键依据
- 示例：eventSubject是@ABONHYANUAE → anySpecificNumbers应包含"粉丝数：@ABONHYANUAE约4.8万"
- 示例：eventSubject是CZ → anySpecificNumbers应包含"粉丝数：CZ约1096万"
