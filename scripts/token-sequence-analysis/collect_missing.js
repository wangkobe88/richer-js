 ...
 * 加载已有数据
 */
function loadExistingData(experimentId) {
  const filepath = path.join(DATA_DIR, `exp_${experimentId.slice(0, 8)}.json`);
  ...
  try {
    ...
    console.log(`   ⚠️ 无法读取已有数据: ${error.message}`);
    return null;
  }
}
 ...
 * 采集单个实验的缺失数据
 */
async function collectMissingData(experimentId) {
  console.log(`\n【实验 ${experimentId.slice(0, 8)}...】`);

  // 加载已有数据
  const existingData = loadExistingData(experimentId);

  let existingTokensMap = {};
  let existingTokens = [];

  if (existingData && existingData.tokens) {
    existingTokens = existingData.tokens;
    existingTokensMap = {};
    existingTokens.forEach(t => {
      existingTokensMap[t.token_address] = t;
    });
    console.log(`   已有数据: ${existingTokens.length} 个代币`);
  }

  // 获取代币列表
  const allTokens = await getExperimentTokens(experimentId);
  ...
  // 找出需要采集的代币（没有数据或数据无效的）
  const tokensToCollect = allTokens.filter(token => {
    const existing = existingTokensMap[token.token_address];
    if (!existing) return true;

    // 检查已有数据是否有效
    const aveResponse = existing.ave_api_response;
    if (!aveResponse || !aveResponse.success || !aveResponse.data) {
      return true;
    }
  ...
  const missingCount = tokensToCollect.length;
  console.log(`   需要采集: ${missingCount} 个代币`);
  ...
  // 采集缺失的数据
  console.log(`   开始采集缺失数据...`);
  const collectedTokens = [];

  let processed = 0;
  let failed = 0;

  for (let i = 0; i < tokensToCollect.length; i += MAX_CONCURRENT_REQUESTS) {
    const batch = tokensToCollect.slice(i, i + MAX_CONCURRENT_REQUESTS);
    ...
    const batchResults = await Promise.all(
      batch.map(async (token) => {
        ...
        if (processed % 10 === 0 || processed === tokensToCollect.length) {
          console.log(`      进度: ${processed}/${tokensToCollect.length} (失败: ${failed})`);
        }
    ...
    collectedTokens.push(...batchResults.filter(r => r !== null));
  }

  // 合并已有数据和新采集的数据
  const finalTokens = [...existingTokens];
  const collectedAddresses = new Set(collectedTokens.map(t => t.token_address));

  // 移除重复的旧数据，使用新采集的数据
  const finalTokensFiltered = finalTokens.filter(t => !collectedAddresses.has(t.token_address));
  finalTokensFiltered.push(...collectedTokens);

  const experimentData = {
    experiment_id: experimentId,
    total_tokens: allTokens.length,
    successful_collected: finalTokensFiltered.length,
    failed: allTokens.length - finalTokensFiltered.length,
    time_window_seconds: TIME_WINDOW_SECONDS,
    tokens: finalTokensFiltered,
    collected_at: new Date().toISOString()
  };

  console.log(`   ✓ 采集完成: ${collectedTokens.length} 新增, ${finalTokensFiltered.length} 总计`);
...
async function main() {
  ...
  for (const experimentId of EXPERIMENT_IDS) {
    try {
      const data = await collectMissingData(experimentId);
