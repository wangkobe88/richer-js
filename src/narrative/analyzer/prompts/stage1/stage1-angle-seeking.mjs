/**
 * Stage 1：找角度推文专用事件预处理 Prompt
 * 用于已被预分类为"找角度推文"的代币分析
 *
 * 核心特点：
 * - 已知是找角度推文，不需要LLM判断推文类型
 * - 推文互动数据已在输入层面排除，无需LLM自行忽略
 * - 比原始Stage 1短~40%，减少LLM认知负担
 */
...
export const STAGE1_ANGLE_SEEKING_VERSION = 'V1.0';

/**
 * 构建找角度推文专用 Stage 1 Prompt
 * @param {Object} tokenData - 代币数据
 * @param {Object} fetchResults - 获取的数据结果
 * @returns {string} Stage 1 Prompt
 */
export function buildStage1AngleSeekingPrompt(tokenData, fetchResults) {
  const {
    twitterInfo = null,
    websiteInfo = null,
    amazonInfo = null,
...
🚨 **⚠️ 推文类型：找角度推文**
- 这条推文已被识别为**找角度推文**：推文作者借用外部热点事件/产品/新闻来发币
- **推文发布者不是事件主体**，只是借势者
- **事件 = 推文陈述的外部事件/现象**（不是"某人发推"这个动作）
- 推文互动数据已从输入中排除，只需关注事件本身的传播/热度数据
...
${accountSummary}`);
  ...
  // 各类语料sections
  const twitterSection = buildTwitterSection(twitterInfo, { excludeEngagement: true });
  ...
  // 分析框架
  sections.push(buildAngleSeekingFramework());
 ...
 * 构建找角度推文专用分析框架
 */
function buildAngleSeekingFramework() {
  return `
╔══════════════════════════════════════════════════════════════════════════════╗
║            Stage 1：找角度推文 — 事件预处理框架                               ║
╚══════════════════════════════════════════════════════════════════════════════╝

【分析目标】
1. 事件描述提取：提取推文引用的外部事件的详细信息
...
⚠️ **必须保留推文作者的叙事构建**：
- 找角度推文中，推文作者经常会**解释为什么选择这个代币名/概念**，这种解释是叙事的核心，必须完整保留在事件描述中
- 包括但不限于：
  - 推文作者对代币名的解读/解释（如"bull也符合BNB主旋律"）
  - 推文作者构建的概念关联（如"这是一场搜索结果革命"）
  - 推文作者阐述的发币理由/逻辑链条
- ❌ **不能只提取外部事件而丢弃推文作者自己的叙事**：推文作者对概念的解读、命名的理由、构建的关联都是事件描述的重要组成部分
...
⚠️ **推文互动数据（点赞、转发、评论数）已从输入中排除**：找角度推文只是借势发币，推文本身的互动数据与事件热度完全无关，因此不提供。

- ✅ **必须提取**：事件主体的影响力数据（粉丝数、认证状态、头衔等）
  - 示例：事件主体是@ABONHYANUAE → 关键数据应包含"粉丝数：@ABONHYANUAE约4.8万"
  - 示例：事件主体是CZ → 关键数据应包含"粉丝数：CZ约1096万"
- ✅ **必须提取**：事件本身的传播/热度数据
  - "在日推非常火爆"、"全网热议"、"千万播放"、"GitHub星标8万"
...

{
  ...
  "eventDescription": {
    ...
    "eventSubject": "事件主体（⚠️ 被引用事件中的参与者，不是推文发布者）",
    ...
    "keyData": {
      "heatLevel": "事件热度描述",
      "spreadData": "事件传播数据（如播放量/阅读量/星标数等）",
      "anySpecificNumbers": "事件具体数字"
    }
  },
  "eventClassification": {
...
⚠️ **keyData 填写示例**：
- ✅ "播放量500万、GitHub星标8万、全网热议" → 事件传播数据
- ✅ "事件主体CZ粉丝约1096万" → 事件主体影响力数据

⚠️ **注意**：
- eventContent将作为Stage 2的输入，请尽量详细
- eventTheme和eventContent中禁止使用"发币"视角
`;
}
