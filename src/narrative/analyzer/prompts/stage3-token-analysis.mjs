 ...
 * Prompt版本号
 */
export const STAGE3_TOKEN_ANALYSIS_PROMPT_VERSION = 'V18.1';
...
如果代币名（Symbol或Name，去除emoji、数字、特殊字符后）与上述品牌名称**完全匹配或高度相似**（包括：完全匹配、以品牌名开头并附加后缀如"BNBARMY"、品牌名的轻微变体如"BTCC"），则：
...
**示例**：
- 代币"BNB🔥"，事件"CZ转发币安历史视频" → BNB是知名代币，事件直接关联币安 → 品牌劫持 → low
- 代币"ETHKING"，事件"V神发言" → ETH是知名代币，ETHKING是蹭品牌变体 → 品牌劫持 → low
- 代币"CZ"，事件"CZ发布新书" → CZ是人名，满足豁免条件 → 不触发品牌劫持，正常评估
- 代币"MUSK"，事件"某KOL评论Musk" → Musk是人名但不满足豁免（非本人发起） → 品牌劫持 → low
- 代币"LINK"，事件"游戏角色Link发布新皮肤" → Link是游戏角色，与Chainlink完全无关 → 不触发品牌劫持，正常评估
