...
**⚠️ 如果是找角度推文，不提取传播数据**：
- 找角度推文的评估重点是"事件主体"的影响力，不是"推文"的传播数据
- 不要提取：推文的点赞、转发、作者粉丝数等传播数据
- 只提取：事件主体相关的数据（如Trump的全球影响力、媒体报道等）
- 示例：找角度推文"Trump posted this baby" → 不提取"230点赞、54转发"，只关注Trump的世界级影响力

**⚠️ 如果不是找角度推文**，按以下规则提取：

- **如果事件主体是超大IP**（isLargeIP=true）：
  - 应该关注IP本身的影响力，而不是当前语料的传播数据
  - 超大IP的示例数据：Twitter/X粉丝数（如Trump有数亿粉丝）、全球媒体关注度、历史传播记录

- **如果事件主体不是超大IP**：
  - 提取当前语料的点赞、转发、播放量等传播数据
  - 热度描述：如"全网热度破百亿"、"登上热搜"、"数百万讨论"
  - 传播数据：如"50w人二创"、"播放量1000万"、"全球媒体报道"
...
**判断优先级**（当事件可能属于多个类别时）：
1. ⚠️ **如果是找角度推文** → 按"推文陈述的事情"分类，不要归为F类
2. 如果是**超大IP的言论** → 优先归为A类
3. 如果是**IP概念推出** → 优先归为B类
4. 如果是**发现型meme** → 归为E类
5. 如果是**日常运营** → 直接阻断
