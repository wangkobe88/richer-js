...
// 获取单个钱包数据
async function getSingleWallet(address) {
  const response = await get(`http://localhost:3010/api/wallets/${address}`);
...
async function main() {
  ...
  try {
    const existingData = JSON.parse(fs.readFileSync(path.join(DATA_DIR, 'wallet_data.json'), 'utf8'));
    existingWalletData = existingData;
    console.log(`  ✓ 现有钱包数据: ${existingWalletData.length} 个`);
  } catch (e) {
    console.log(`  ⚠️  无法加载现有钱包数据`);
  }

  // 建立现有钱包地址映射
  const existingWalletMap = new Map();
  existingWalletData.forEach(w => existingWalletMap.set(w.address.toLowerCase(), w));

  // 找出缺失的钱包
  const missingWallets = Array.from(allWallets).filter(addr => !existingWalletMap.has(addr.toLowerCase()));
  console.log(`  ⚠️  缺失钱包: ${missingWallets.length} 个`);

  // 补充获取缺失的钱包数据
  if (missingWallets.length > 0) {
    console.log(`  [补充获取缺失钱包数据...]`);
    let fetchedCount = 0;
    for (let i = 0; i < Math.min(missingWallets.length, 500); i++) {
      const wallet = await getSingleWallet(missingWallets[i]);
      if (wallet) {
        existingWalletMap.set(wallet.address.toLowerCase(), wallet);
        fetchedCount++;
      }
      if ((i + 1) % 50 === 0) {
        process.stdout.write(`\r    进度: ${i + 1}/${Math.min(missingWallets.length, 500)} (获取到: ${fetchedCount})`);
      }
    }
    console.log(`\r    ✓ 完成: 获取到 ${fetchedCount} 个钱包`);
  }

  // 合并所有钱包数据
  const allWalletData = Array.from(existingWalletMap.values());
  console.log(`  ✓ 总钱包数据: ${allWalletData.length} 个`);
  ...
  fs.writeFileSync(
    path.join(DATA_DIR, 'final_analysis_results_v2.json'),
    JSON.stringify(result, null, 2)
  );

  console.log('\n✅ 分析完成!');
  console.log(`数据保存在: ${DATA_DIR}/final_analysis_results_v2.json`);
}
