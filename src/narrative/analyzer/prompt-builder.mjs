...
- **如果有推文且推文仅为链接（如只有"https://t.co/xxx"）**：
  - **继续到第四步判断intro/网页内容**，不要直接返回unrated
  - 原因：intro或网页内容可能提到了知名概念/事物，仍然是有效叙事
- **如果有推文且推文包含链接/图片和其他内容**：
  - 推文明确提到代币名称或关联概念 → 继续评分
  - 推文未提及代币，但intro/网页内容提到知名概念 → 继续到第四步判断
  - 推文无意义且intro/网页内容也无法理解 → 返回 unrated
...
- **借用概念/名称→评估概念知名度**：intro/网页内容提到某个事物/概念
  - **关键**：meme币的核心就是"借用"概念/名称，但需要评估被借用概念的**知名度/影响力**
  - **判断标准**：
    1. 如果内容提到的是**世界级知名概念**（Tesla、ChatGPT、iPhone、抖音、比特币等）→ 高叙事潜力，可以high/mid
    2. 如果内容提到的是**普通/小众概念**（某个开源工具、普通产品、地方性事物）→ 叙事价值有限，通常low
    3. 如果内容无法体现该概念的**任何知名度/影响力**→ unrated（不知道这是什么，无法评估传播潜力）
  - **示例**：
    - 代币名"ChatGPT"，网页提到"OpenAI的ChatGPT AI助手"→ 世界级知名概念 → 可评分
    - 代币名"OpenShell"，网页仅说"一个开源运行时工具"→ 小众概念，无知名度体现 → unrated/low
    - 代币名"Tesla"，网页提到"特斯拉电动车，全球领先"→ 世界级知名概念 → 可评分
  - **注意**：不要因为内容"提到"了某个概念就认为是有效叙事，关键在于这个概念的**知名度**
