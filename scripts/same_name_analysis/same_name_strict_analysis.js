 ...
 *
 * 匹配规则：
 * 1. name 完全相同（忽略大小写和首尾空格）
 * 2. 或者 name1 包含 name2 的主要部分
 * 3. 或者 name2 包含 name1 的主要部分
 */
...
async function analyzeSameNameTokens(token, aveAPI) {
  ...
  try {
    ...
    const strictSameNameTokens = results.filter(t => {
      ...
      return isSameName(tokenName, symbol, t.name || '', t.symbol || '');
    });

    // 排除自己
    const excludeSelf = strictSameNameTokens.filter(t => t.token !== tokenAddress);

    // 统计链
    const chains = [...new Set(strictSameNameTokens.map(t => t.chain))];

    // 找到最大FDV
    const fdvList = strictSameNameTokens.map(t => parseFDV(t.fdv)).filter(v => v > 0);
    const marketCapList = strictSameNameTokens.map(t => parseFDV(t.market_cap)).filter(v => v > 0);
    ...
    const result = {
      ...
      strict_same_name_count: strictSameNameTokens.length,
      ...
      chains: chains,
      same_name_tokens: strictSameNameTokens.map(t => ({
        token: t.token,
        symbol: t.symbol,
        name: t.name || '',
        chain: t.chain,
      ...
      filtered_out_count: results.length - strictSameNameTokens.length
    };

    console.log(`      ✅ 搜索 ${results.length} 个, 严格匹配 ${strictSameNameTokens.length} 个 (排除自己: ${excludeSelf.length}, 过滤掉: ${result.filtered_out_count})`);
...
function summarizeByQuality(analyzedTokens) {
  ...
  for (const [quality, tokens] of Object.entries(byQuality)) {
    ...
    const counts = tokens.map(t => t.strict_same_name_count);
...
function printReport(analyzedTokens, summary) {
  ...
  const totalSameName = analyzedTokens.filter(t => t.strict_same_name_count > 1).length;
  ...
  console.log(`   有严格同名代币: ${totalSameName} (${(totalSameName/totalCount*100).toFixed(1)}%)`);
  console.log(`   有严格同名(排除自己): ${totalSameNameExcludeSelf} (${(totalSameNameExcludeSelf/totalCount*100).toFixed(1)}%)`);
  ...
  qualityOrder.forEach(quality => {
    ...
    console.log(`   平均严格同名数: ${data.avg_count.toFixed(1)}, 中位数: ${data.median_count}`);
  ...
  analyzedTokens.forEach(t => {
    const count = t.strict_same_name_count;
  ...
  // 找出严格同名代币最多的案例
  const mostSameName = [...analyzedTokens].sort((a, b) => b.strict_same_name_count - a.strict_same_name_count)[0];
  if (mostSameName && mostSameName.strict_same_name_count > 1) {
    console.log(`\n   严格同名代币最多: ${mostSameName.token_symbol} (${mostSameName.token_name})`);
    console.log(`   同名数量: ${mostSameName.strict_same_name_count} 个 (搜索结果: ${mostSameName.search_result_count}, 过滤: ${mostSameName.filtered_out_count})`);
  ...
  if (maxFDV && maxFDV.max_fdv > 0) {
    console.log(`\n   最大严格同名代币FDV: ${maxFDV.token_symbol} (${maxFDV.token_name})`);
    console.log(`   最大FDV: $${formatNumber(maxFDV.max_fdv)}`);
    console.log(`   质量: ${maxFDV.quality_category}`);
    console.log(`   同名数量: ${maxFDV.strict_same_name_count} 个`);
  }

  // 找出过滤比例最高的案例
  const mostFiltered = [...analyzedTokens]
    .filter(t => t.search_result_count > 0)
    .sort((a, b) => (b.filtered_out_count / b.search_result_count) - (a.filtered_out_count / a.search_result_count))[0];

  if (mostFiltered && mostFiltered.filtered_out_count > 0) {
    console.log(`\n   过滤比例最高: ${mostFiltered.token_symbol} (${mostFiltered.token_name})`);
    console.log(`   搜索结果: ${mostFiltered.search_result_count} 个, 过滤: ${mostFiltered.filtered_out_count} 个 (${(mostFiltered.filtered_out_count/mostFiltered.search_result_count*100).toFixed(1)}%)`);
    console.log(`   质量: ${mostFiltered.quality_category}`);
  }
  ...
  if (Math.abs(highAvgCount - lowAvgCount) > 2) {
    console.log(`\n   ✅ 严格同名代币数量也有区分度 (高质量平均${highAvgCount.toFixed(1)} vs 低质量${lowAvgCount.toFixed(1)})`);
  } else {
    console.log(`   ❌ 严格同名代币数量区分度不明显 (高质量平均${highAvgCount.toFixed(1)} vs 低质量${lowAvgCount.toFixed(1)})`);
  }

  const avgFilterRate = totalFiltered / analyzedTokens.reduce((sum, t) => sum + t.search_result_count, 0) * 100;
  console.log(`\n   📊 名称匹配过滤率: ${avgFilterRate.toFixed(1)}% (${totalFiltered}个被过滤)`);
  console.log(`   💡 严格匹配能有效去除虚假的"同名"代币`);
}
  ...
  try {
    ...
    const outputData = {
      ...
      matching_rules: {
        ...
        rules: [
          'name 完全相同（忽略大小写和空格）',
          '或者 name 互相包含（处理 "Leo" vs "Leo Token"）',
          '或者 symbol 相同且 name 包含相同的核心词'
        ]
      }
    };

    const fs = require('fs');
    const outputFile = `same_name_strict_analysis_${EXPERIMENT_ID.substring(0, 8)}_${Date.now()}.json`;
