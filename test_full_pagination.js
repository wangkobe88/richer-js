...
async function testFullPagination() {
  ...
  let currentToTime = checkTime; // 使用 let 而不是 const

  console.log('=== 完整测试回测引擎的分页逻辑 ===\n');
  ...
  while (loopCount < maxLoops) {
    ...
    try {
      ...
      if (trades.length === 300) {
        ...
        console.log(`  新的 toTime (格式化): ${new Date(currentToTime * 1000).toLocaleString()}`);
      } else {
        console.log('\n✓ 返回数据不足300条，已获取完毕');
        break;
      }
  ...
  if (allTrades.length > 0) {
    ...
    if (firstTime > targetFromTime) {
      ...
      console.log('  第二次查询应该填充这个缺失，但返回了0条数据');
    } else {
      console.log('\n✓ 数据完整');
    }
  }

  // 分析：如果只查询了一次且返回300条，说明有问题
  if (loopCount === 1 && allTrades.length === 300) {
    console.log('\n❌ 问题确认：');
    console.log('  只查询了1次，返回了300条数据');
    console.log('  但最早的交易时间比目标起始时间晚了15秒');
    console.log('  说明第二次查询没有执行，或者第二次查询返回了空数据');
    console.log('\n可能的原因：');
    console.log('  1. 在 targetFromTime 到 currentToTime 之间确实没有交易');
    console.log('  2. API 在特定参数下返回空数据');
    console.log('  3. 回测引擎配置有误');
  }
}
