 ...
 * 功能：
 * 1. 详细事件描述提取
 * 2. 初步分类判断（A-F类）- 采用"内容本质优先级"
 * 3. 性质标记识别（推测性/发现型/营销性）
 * 4. 找角度推文识别（内部规则，不输出）
 *
 * V4.0 修改：
 * - 新增F类（发现型叙事类）：将E类中的"发现型meme"独立为单独类别
 * - E类精简为纯"社会热点/现象类"，不再包含发现型内容
 * - 优先级调整为 A > W > B > F > C > D > E
 * - 分类从5类(A-E)+W 扩展为 6类(A-F)+W
 *
 * V3.0 修改：
 * - 移除"空洞内容检查"步骤：所有通过预检查的语料都能定义事件
 * - 添加兜底事件定义规则：无法用已有方法定义时，使用"@[作者]发推'[内容]'"格式
 * - 空洞内容的价值评估由 Stage 2（C类维度二阻断条件）负责
 * - 步骤从4步精简为3步
 */
 ...
 * Prompt版本号
 */
export const STAGE1_EVENT_PREPROCESSING_VERSION = 'V4.0';
...
2. 初步分类判断：判断事件属于哪个类别（A-E类）
3. 性质标记识别：识别事件是否有推测性/发现型/营销性等性质
...
5️⃣ C类（人物相关） - 个人账号的言论/动作/互动（机构官方账号不归此类）
6️⃣ D类（机构相关） - 机构官方账号的言论/动作/互动
7️⃣ E类（社会现象） - 群体性的热点、社会现象
...
- **F类 vs C类的边界**：如果推文只是在引用/报道某人说的话 → C类；如果推文在关联多个信息点揭示隐藏叙事 → F类
...

{
  ...
  "eventClassification": {
    "primaryCategory": "A/W/B/F/C/D/E",
    "primaryCategoryName": "形象化IP/Web3项目/产品相关/发现型叙事/人物言论动作/机构言论动作/社会热点现象",
