...
const txApi = new AveTxAPI(
  process.env.AVE_API_URL || 'https://prod.ave-api.com',
  30000,
...
async function getPlatformFromRawApi(rawApiData) {
  if (!rawApiData) return null;
  try {
    const parsed = typeof rawApiData === 'string' ? JSON.parse(rawApiData) : rawApiData;
    // 检查platform字段或从pair地址推测
    if (parsed.platform) return parsed.platform;
    const mainPair = parsed.main_pair || parsed.token?.main_pair;
    if (mainPair) {
      if (mainPair.endsWith('_fo')) return 'fourmeme';
      if (mainPair.endsWith('_iportal')) return 'flap';
    }
    return 'fourmeme'; // 默认
  } catch (e) {
    return null;
...
// 直接调用 AveTxAPI 获取交易
async function fetchEarlyTrades(tokenAddress, platform, launchAt) {
  try {
    // 构造内盘 pair
    let innerPair;
    ...
    const trades = await txApi.getSwapTransactions(
      pairId,
      300,                      // limit
      launchAt,                 // fromTime - 代币创建时间
      launchAt + 600,           // toTime - 代币创建后10分钟
      'asc'                     // sort
    );
...
async function main() {
  ...
  data.forEach(t => {
    ...
    if (judges && judges.category) {
      ...
      const platform = getPlatformFromRawApi(t.raw_api_data);
      if (launchAt && platform) {
        judgedTokens.push({
  ...
  for (const category of categories) {
    ...
    for (let i = 0; i < tokens.length; i++) {
      ...
      const apiResult = await fetchEarlyTrades(token.tokenAddress, token.platform, token.launchAt);
