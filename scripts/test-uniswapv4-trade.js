/**
 * UniswapV4 交易测试脚本（通用版）
 * 自动发现 hooks，无需手动指定 PoolKey
 *
 * 用法: node scripts/test-uniswapv4-trade.js
 */
...
const UniswapV4Trader = require('../src/trading-engine/traders/implementations/UniswapV4Trader');

// ============ 配置 ============
const EXPERIMENT_ID = '60ec9bf6-5a74-427c-a029-0db2b12defef';
const TOKEN_ADDRESS = '0x5cc0846ea203ffdad359ad4c31a7dfb2f62e1110'; // CLIPPY (Livo)
const TRADE_AMOUNT = '0.001'; // ETH
// ==============================

async function main() {
  console.log('=== UniswapV4 交易测试（通用版）===\n');
  ...
  // 3. 创建交易器
  console.log('[3/8] 初始化 UniswapV4 交易器...');
  const trader = new UniswapV4Trader({
    blockchain: 'ethereum',
    slippage: 0.05,
    maxSlippage: 0.10,
    gasLimit: 500000,
  ...
  console.log(`  CLIPPY 余额: ${tokenBalanceBefore}`);

  // 5. 买入
  console.log(`\n[5/8] 买入 ${TRADE_AMOUNT} ETH 的 CLIPPY...`);
  ...
  console.log(`  CLIPPY 余额: ${tokenBalanceAfter}`);

  // 7. 卖出全部代币
  console.log(`\n[7/8] 卖出全部 CLIPPY (${tokenBalanceAfter})...`);
  ...
  console.log(`  CLIPPY 余额: ${finalTokenBalance}`);

  // 计算损耗
  const ethDiff = parseFloat(ethBalance) - parseFloat(finalEthBalance);
  console.log(`\n  总 ETH 损耗(含 gas): ${ethDiff.toFixed(6)} ETH`);
