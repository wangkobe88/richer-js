...
console.log(`🧹 继续清理实验 ${experimentId} 的剩余孤儿数据...\n`);
...
// 找出孤儿代币
console.log('步骤3: 识别剩余孤儿代币...');
const orphans = uniqueTokensFromTS.filter(addr => !tokensInTable.has(addr));
console.log(`   发现 ${orphans.length} 个剩余孤儿代币\n`);

if (orphans.length === 0) {
  console.log('✅ 没有剩余孤儿数据，清理完成！');
  process.exit(0);
}
...
// 分批删除（使用更小的批次）
console.log('步骤4: 分批删除剩余孤儿时序数据...');
const deleteBatchSize = 50; // 减小批次大小
let deletedCount = 0;
