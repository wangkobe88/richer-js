 ...
 * Prompt版本号
 */
export const STAGE3_TOKEN_ANALYSIS_PROMPT_VERSION = 'V18.4';
...
⚠️ **品牌劫持/拼写检查的门控逻辑**：
- 如果品牌劫持检查（1.0）触发且**不满足豁免条件** → 直接返回 low，**跳过** 1.1-1.3 评分
- 如果拼写错误检查（1.0.1）触发 → 直接返回 low，**跳过** 1.1-1.3 评分
- 如果品牌劫持检查未触发，或触发但**满足豁免条件** → 进入 1.1-1.3 正常评分流程

⚠️ **泛化概念不构成有效关联**（适用于1.1-1.3所有匹配层级）：
- 极度泛化的概念（金钱、自由、成功、胜利、爱、快乐、和平等）不构成有效关联
- 这些概念可以关联到几乎任何事件，缺乏排他性
- 示例：代币"自由"，事件"某公司发布新产品" → "自由"太泛化 → 不构成有效关联
...
⚠️ **关联性底线提醒**：在第三步确定category时，如果关联性得分 ≤ 10分，category必须为"low"（详见第三步决策树）。因此只有强文化关联和中上等文化关联能越过底线，弱文化关联和无关联仍为low。
...
**决策节点2：质量底线检查（仅当关联性 > 10分时执行）**
- 查看第二步的质量得分（长度 + 拼写/可读性 + 名称合理性）
- **如果质量得分 ≤ 4分 → category = "low"，到此结束**
- 理由：代币名称质量极差（拼写错误多、完全无意义等），即使有关联也不值得推荐
- 只有关联性 > 10分 且 质量得分 > 4分时，才继续往下

**决策节点3：总分计算（仅当关联性 > 10分 且 质量 > 4分时执行）**
- 事件分 = Stage 2总分 × 0.6
...
- 总分 = 51 + 20 + 15 = 86，关联分20 > 10，质量分15 > 4 → 按总分分类 → **high**

❌ 关联性不达标（注意：即使总分 ≥ 70，category也必须是low）：
- 事件分：85 × 0.6 = 51，关联分：0，质量分：19
- 总分 = 51 + 0 + 19 = 70，但关联分0 ≤ 10 → **low**（不看总分和质量）

❌ 质量不达标（关联性达标但质量极差）：
- 事件分：85 × 0.6 = 51，关联分：15，质量分：3（名称完全无意义+拼写错误多）
- 总分 = 51 + 15 + 3 = 69，关联分15 > 10，但质量分3 ≤ 4 → **low**（不看总分）

⚠️ **警告**：关联性≤10分或质量≤4分时，无论总分是多少，category都必须填"low"。

⚠️ **category 必须严格按决策树结果填写，禁止基于主观判断覆盖**：
- 算出分数后，严格按决策树逻辑判定 category
- 禁止出现 total_score=68 但 category="low" 的情况（应该为"mid"）
- 禁止出现 total_score=75 但 category="mid" 的情况（应该为"high"）
- 如果你对分数有异议，应在分数层面调整，而不是在 category 层面覆盖决策树

⚠️ **关键原则：第一步的关联性得分在第三步不可重新评估**：
- 第一步确定的关联性得分是最终值，在第三步必须直接使用
- 第三步的决策树是纯数学映射，不涉及主观判断
- 禁止出现"关联性12分>10（通过底线），但我觉得关联弱所以category=low"的情况
- 如果你认为关联性应该更低，应该在第一步调整分数，而不是在第三步推翻决策树
- 即：要么第一步给 ≤10 分（直接low），要么第一步给 >10 分（严格按总分分类），不存在中间地带
