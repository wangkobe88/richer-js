/**
 * Prompt构建器 - V7.14 Section-based架构
 * 根据实际获取的数据动态组装Prompt
 *
 * V7.14变更：
 * - 新增低质量场景：纯负面概念/负面事件
...
**1. 硬蹭/弱关联**：代币名称包含推文/叙事内容中没有的内容

🚫 **以下情况跳过硬蹭检查（直接进入评分步骤）：**
- 有YouTube/抖音/TikTok/Bilibili视频
- 理由：根据分析原则，视频存在时假设视频内容与代币相关（无法解析视频内容），不能仅根据标题/推文不匹配就判定硬蹭

✅ **仅当无视频时，执行以下硬蹭检查：**
- 检查方法：比较代币名称和推文/叙事内容，代币名是否包含了推文中没有的关键词？
- 示例：推文只提到"绿水青山"，代币名却是"绿水青山就是金山银山" → 硬蹭，直接low
- 示例：推文只是"Cute 🥰"，代币名是某个不相关的中文词 → 硬蹭，直接low
...
**7. 语言不匹配导致传播障碍**：推文语言与代币名称语言不匹配
   ...
   - 示例：推文是中文，代币名是英文/其他非中文语言 → 直接low
   - 理由：目标受众无法理解、无法记住、无法传播不同语言的代币名
   - 例外：代币名是英文/中文通用词（如"猫"、"Dog"等）
