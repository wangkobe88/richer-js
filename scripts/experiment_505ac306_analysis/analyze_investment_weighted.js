...
console.log('  钱包数:', walletDataComplete.length);
...
async function getTokenReturnsAndLabels() {
  ...
  const tokenResults = tokenAddresses.map(tokenAddress => {
    ...
    const returnRate = ((remainingCost - totalCost) / totalCost) * 100;
...
const detailedScheme = {
  ...
  name: "细致十分类",
  categories: [
    { name: "🏆 顶层巨鲸", priority: 1, rules: [{ dimension: "balance", min: balanceP[95] }] },
    { name: "💰 大户", priority: 2, rules: [{ dimension: "balance", min: balanceP[90], max: balanceP[95] }] },
    { name: "💼 中大户", priority: 3, rules: [{ dimension: "balance", min: balanceP[75], max: balanceP[90] }] },
    { name: "⚡ 高频交易者", priority: 4, rules: [{ dimension: "trades", min: tradeP[80] }] },
    { name: "🌟 新晋活跃", priority: 5, rules: [{ dimension: "age", max: ageP[20] }, { dimension: "trades", min: tradeP[75] }] },
    { name: "👴 老玩家", priority: 6, rules: [{ dimension: "age", min: ageP[80] }] },
    { name: "👔 中产玩家", priority: 7, rules: [{ dimension: "balance", min: balanceP[50], max: balanceP[75] }] },
    { name: "🐟 小户活跃", priority: 8, rules: [{ dimension: "balance", min: balanceP[25], max: balanceP[50] }, { dimension: "trades", min: tradeP[40] }] },
    { name: "🐠 小户", priority: 9, rules: [{ dimension: "balance", min: balanceP[25], max: balanceP[50] }] },
    { name: "🐟 散户", priority: 10, rules: [{ dimension: "balance", max: balanceP[25] }] }
  ]
};
...
async function main() {
  ...
  console.log('基于人工标注质量的分析 v5（使用实际投入金额作为权重）');
  console.log('='.repeat(80));
  ...
  labeled.forEach(token => {
    ...
    const investmentData = investmentMap[tokenAddr];

    if (!investmentData || investmentData.wallet_count === 0) return;

    const walletInvestments = investmentData.wallet_investments || {};
    const totalInvestment = investmentData.total_investment_usd || 0;

    // 统计各分类加权占比（按实际投入金额）
    const categoryWeightedSum = {};

    Object.keys(walletInvestments).forEach(addr => {
      const cat = walletClassMap[addr] || '未知';
      const weight = walletInvestments[addr]; // 该钱包在这个代币中的投入金额
      categoryWeightedSum[cat] = (categoryWeightedSum[cat] || 0) + weight;
    });

    const categoryRatios = {};
    Object.keys(categoryWeightedSum).forEach(cat => {
      categoryRatios[cat] = categoryWeightedSum[cat] / totalInvestment;
    });
    ...
    analysisResults.push({
      ...
      participant_count: investmentData.wallet_count,
  ...
  // 计算相关性
  console.log('\n' + '='.repeat(80));
  console.log('钱包分类占比与质量的相关性分析（实际投入金额加权）');
  console.log('='.repeat(80));
  ...
  Object.keys(categoryData).forEach(cat => {
    ...
    const spearman = require('./analyze_volume_weighted.js').spearmanCorrelation ?
      require('./analyze_volume_weighted.js').spearmanCorrelation(data.weightedRatios, data.qualityValues) :
      null;
  ...
  // 找出最相关的分类
  console.log('\n' + '='.repeat(80));
  console.log('关键发现（按二分类相关性排序）');
  console.log('='.repeat(80));

  const sortedByCorrelation = Object.entries(correlations)
    .filter(([_, d]) => d.point_biserial !== null)
    .sort((a, b) => Math.abs(b[1].point_biserial) - Math.abs(a[1].point_biserial));
  ...
  // 各质量组的平均构成
  console.log('\n' + '='.repeat(80));
  console.log('各质量组的平均钱包构成（实际投入金额加权）');
  console.log('='.repeat(80));
  ...
  ['high_quality', 'mid_quality', 'low_quality'].forEach(quality => {
    ...
    detailedScheme.categories.forEach(cat => {
      ...
      if (avgRatio > 0.001) {
        catAvgRatios[cat.name] = avgRatio;
      }
    });

    Object.entries(catAvgRatios)
      .sort((a, b) => b[1] - a[1])
      .forEach(([cat, ratio]) => {
        console.log(`  ${cat}: ${(ratio * 100).toFixed(1)}%`);
      });
  });
  ...
  fs.writeFileSync(
    path.join(DATA_DIR, 'data/quality_investment_weighted_results.json'),
    JSON.stringify(result, null, 2)
  );

  console.log(`\n✅ 分析完成! 结果已保存到 data/quality_investment_weighted_results.json`);
}
