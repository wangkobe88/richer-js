/**
 * 超大IP快速通道 — 共享辅助函数
 * 供 person-prompt.mjs 和 institution-prompt.mjs 共用
 */

/**
 * Prompt版本号
 */
export const SUPER_IP_FAST_PROMPT_VERSION = 'V1.1';
 ...
 * 构建截断规则
 */
export function buildTruncationRules(includeBrandCheck) {
  return includeBrandCheck
 ...
 * 构建语料数据段落
 */
export function buildDataSections(fetchResults) {
 ...
 * 构建品牌劫持检查段落（精简版）
 */
export function buildBrandHijackSection(ipType) {
  return `
...
- 豁免（${ipType === 'person' ? '如代币名是本人名' : '人物名'}）：事件确系该人物本人直接发起，有实质内容，有发酵空间
...
/**
 * 构建共用头部（已知信息 + 代币信息 + 评估原则）
 */
export function buildCommonHeader(ipInfo, preScores, tokenData, dataSections, categoryLabel) {
...
/**
 * 构建共用代币评估段落（关联性 + 质量 + 截断规则 + 分数聚合 + 输出格式）
 */
export function buildCommonTokenSection(ipInfo, brandSection, truncationRules, preScores) {
  return `**代币-事件关联性评估（0-20分）**
${brandSection}
