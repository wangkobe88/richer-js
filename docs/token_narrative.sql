...
create index IF not exists idx_token_narrative_final_rating on public.token_narrative using btree (((stage_final_result ->> 'rating'::text))) TABLESPACE pg_default;

create index IF not exists idx_token_narrative_stage1_finished_at on public.token_narrative using btree (((stage1_result ->> 'finishedAt'::text)) desc) TABLESPACE pg_default
where
  (stage1_result is not null);